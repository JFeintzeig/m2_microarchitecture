...
int counters_init(const event_alias *profile_events, const usize ev_count, uint32_t *classes, usize *counter_map) {
  ...
  memset(ev_arr, 0, ev_count*sizeof(kpep_event*));
  ...
  if ((ret = kpep_config_kpc_map(cfg, counter_map, sizeof(usize) * KPC_MAX_COUNTERS))) {
...
int get_thread_counters(u32 tid, u32 buf_count, u64 *buf) {
  return kpc_get_thread_counters(tid, buf_count, buf);
}
